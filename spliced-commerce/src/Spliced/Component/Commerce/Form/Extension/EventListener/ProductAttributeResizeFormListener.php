...
/**
 * ProductAttributeResizeFormListener
 *
 * @author Gassan Idriss <ghassani@gmail.com>
 */
class ProductAttributeResizeFormListener extends ResizeFormListener
{
    ...
    public function preSubmit(FormEvent $event)
    {
        ...
        if ($this->allowAdd) {
            foreach ($data as $name => $value) {
                if (!$form->has($name)) {
                    $form->add($name, new ProductAttributeType($this->getConfigurationManager(), $this->getProduct()), array_replace(array(
                            'property_path' => '['.$name.']',
                    ), $this->options));
                }
            }
