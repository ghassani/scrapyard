...
function SplicedProjectNote ( element, options ) {

    if (typeof element == 'undefined') {
        console.log('Could Not Find Note Wrapper');
        return;
    }
    ...
    this.settings = $.extend( {
        base_uri : false,
        selector : {
            list   : '#project-note-table',
            add    : '.project-note-new',
            edit   : '.project-note-edit',
            remove : '.project-note-remove'
        }
    }, options );
    this.init();
}

$.extend(SplicedProjectNote.prototype, SplicedProjectManagerBaseClass.prototype, {
    init: function () {
    ...
    add : function(event, element) {
        ...
        $.ajax({
            url : this.settings.base_uri + '/note/new',
            method : 'GET',
            complete: function(){
    ...
    edit : function(event, element) {
        ...
        $.ajax({
            url : this.settings.base_uri + '/note/' + noteId + '/edit',
            method : 'GET',
            complete: function(){
                self.setWorking(false);
            },
                ...
                $modal.find('form').bind('submit', function(e){
                    ...
                    $.ajax({
                        ...
                        success : function(updateResponse) {
                            ...
                            self.element.find('#project-note-' + noteId ).replaceWith($content);
    ...
    remove : function(event, element) {
        ...
        $.ajax({
            url: this.settings.base_uri + '/note/' + $(element).data('id') + '/remove',
            method: 'GET',
            complete: function () {
                self.setWorking(false);
            },
            success : function(response) {
                if (response.success) {
                    $('#project-note-' + $(element).data('id')).fadeOut('slow', function(){
                        $(this).remove();
                    });
                }
            }
